    ...
    execute (inputs, options, cb) {
      ...
      return requestAsync({
        ...
        if (!httpResponse.body || httpResponse.statusCode !== 200) {
          reporter.errorNotification('scriptmanager error - ' + tenant, JSON.stringify(body), new Error(httpResponse.body || 'Executing script failed'))

          if (httpResponse.body && httpResponse.body.toString().indexOf('socket hang up') !== -1) {
            throw new Error('The communication with your dedicated javascript container crashed. This is usually caused by reaching provided resources limits. The container is now about to be restarted.' + httpResponse.body)
          }
