...
module.exports = function (reporter) {
  ...
  app.post('/license-key', (req, res, next) => {
    ...
    return request.post({
      ...
      if ((new Date().getTime() - start) > 2000) {
        logger.error('Long verification processing ' + req.body)
      }
  ...
  app.post('/license-usage', (req, res, next) => {
    ...
    return request.post({
      ...
      if ((new Date().getTime() - start) > 2000) {
        logger.error('Long usage-check processing ' + JSON.stringify(req.body))
      }
    }).pipe(res)
  })
    ...
    try {
      ...
      if (!plan) {
        ...
        return request.post({
          ...
          if ((new Date().getTime() - start) > 2000) {
            logger.error('Long gumroad-hook processing ' + JSON.stringify(body))
          }
        }).pipe(res)
      }
