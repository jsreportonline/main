...
module.exports = function (reporter) {
  ...
  app.use((req, res, next) => {
    if (req.host && req.host.split('.').length >= (subdomainCount - 1)) {
      const domains = req.host.split('.')
  ...
  app.post('/gumroad-hook', function (req, res, next) {
    logger.debug('processing gumroad hook: ' + JSON.stringify(req.body))
    try {
  ...
  app.post('/register', bodyParser.urlencoded({ extended: true, limit: '2mb' }), function (req, res, next) {
    ...
    reporter.multitenancyRepository.findTenantByName(req.body.name).then(() => {
  ...
  app.post('/reset', function (req, res, next) {
    logger.debug(`rest password for ${req.body ? req.body.username : 'unknown'}`)
...
//authenticate basic if request to API
  ...
  app.use(function (req, res, next) {
    reporter.logger.info('Processing ' + req.method + ' ' + req.path, { tenant: req.tenant.name })
    next()
  })
