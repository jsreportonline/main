...
const createRemoteRequestHandler = require('jsreport-worker-docker-manager/lib/remoteRequestHandler')
...
module.exports = function (reporter) {
  ...
  // because jo is using custom authentication code that prevents worker-docker-manager route
  ...
  app.post('/api/worker-docker-manager', (req, res, next) => {
    ...
    return createRemoteRequestHandler(
      ...
      {
        onRequestFilter: async ({ requestBody, renderReq }) => {
          const tenantName = requestBody.tenantName

          if (!tenantName) {
            throw new Error('Missing tenant information in worker')
          }

          const tenant = await reporter.multitenancyRepository.findTenantByName(tenantName)

          if (!tenant) {
            throw new Error('Invalid tenant received in worker')
          }

          renderReq.context = renderReq.context || {}
          renderReq.context.tenant = tenant
          renderReq.context.user = tenant
        },
        onResponseFilter: async ({ type, responseBody }) => {
          if (type === 'scriptManager') {
            if (
              responseBody.request &&
              responseBody.request.context
            ) {
              responseBody.request = Object.assign({}, responseBody.request)
              responseBody.request.context = Object.assign({}, responseBody.request.context)
              delete responseBody.request.context.tenant
              delete responseBody.request.context.user
            }
          } else if (type === 'recipe') {
            if (
              responseBody.req &&
              responseBody.req.context
            ) {
              responseBody.req = Object.assign({}, responseBody.req)
              responseBody.req.context = Object.assign({}, responseBody.req.context)
              delete responseBody.req.context.tenant
              delete responseBody.req.context.user
            }
          }
        }
      }
