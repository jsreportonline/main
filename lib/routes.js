...
module.exports = function (reporter) {
  ...
  app.post('/api/worker-docker-manager', textParser({
    ...
    try {
      const result = await reporter.dockerManager.executeWorker(serializator.parse(req.body))
  ...
  app.post('/license-key', (req, res, next) => {
    logger.info('license key processing')
    ...
    return request.post({
      ...
      logger.info('License key processing ended in ' + time)
  ...
  app.post('/license-usage', (req, res, next) => {
    logger.info('License usage processing')
    const start = new Date().getTime()
    ...
    return request.post({
      ...
      const time = new Date().getTime() - start
      logger.info('License usage processing ended in ' + time)
      if (err && err.code === 'ECONNREFUSED') {
        next(err)
      }
    ...
    try {
      ...
      if (!plan) {
        ...
        return request.post({
          ...
          const time = new Date().getTime() - start
          logger.info('Gumroad hook processing ended in ' + time)
  ...
  passport.serializeUser((tenantOrUser, done) => {
    done(null, tenantOrUser.isAdmin ? tenantOrUser.email : tenantOrUser.name)
  })
  ...
  app.post('/login', bodyParser.urlencoded({ extended: true, limit: '2mb' }), (req, res, next) => {
    ...
    passport.authenticate('local', (err, tenantOrUser, info) => {
      ...
      logger.debug(`login successfull ${tenantOrUser.isAdmin ? tenantOrUser.email : tenantOrUser.name} ${tenantOrUser.isAdmin}`, { tenant: tenantOrUser.tenantId })

      req.session.viewModel = {}

      logger.debug(`log in ${tenantOrUser.isAdmin ? tenantOrUser.email : tenantOrUser.name}`, { tenant: tenantOrUser.tenantId })
