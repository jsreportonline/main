...
const createWorkerTempfileRequestHandler = require('jsreport-docker-workers/lib/workerTempfileHandler')
...
module.exports = function (reporter) {
  ...
  // because jo is using custom authentication code that prevents docker-workers route
  // to pass authentication
  app.get('/api/worker-docker-temp-file', (req, res, next) => {
    const auth = Buffer.from(req.headers.authorization.split(' ')[1], 'base64').toString()
    const username = auth.split(':')[0]
    const password = auth.split(':')[1]
    ...
    return createWorkerTempfileRequestHandler(reporter)(req, res, next)
  })
