...
module.exports = function (reporter) {
  ...
  app.delete('/api/payments/subscription', async (req, res, next) => {
    return request({
      url: `http://host.docker.internal:3000/api/payments/customer/${req.user.tenant.payments.customer.uuid}/subscription/${req.user.tenant.payments.customer.product.id}`,
      method: 'DELETE'
    }, (err, resp, body) => {
      if (err) {
        next(err)
