...
module.exports = function (reporter) {
  ...
  app.post('/gumroad-hook', async (req, res, next) => {
    ...
    try {
      ...
      if (!tenantName) {
        const tenant = await reporter.multitenancyRepository.findTenant(req.body.email)
        if (!tenant) {
  ...
  app.post('/api/account-email', async (req, res, next) => {
    logger.debug(`change-account-email ${req.user.name}`)
    ...
    try {
      ...
      const tenantWithEmail = await reporter.multitenancyRepository.findTenant(req.body.newEmail)

      if (tenantWithEmail) {
        return res.send({ code: 'Email is already used by another account' })
      }

      await reporter.multitenancyRepository.updateTenant(req.user.name, { $set: { email: req.body.newEmail } })
  ...
  app.post('/api/tenant-windows-stopped-inform', async (req, res, next) => {
    logger.debug(`/api/tenant-windows-stopped-inform for tenant (${req.user.name})`)

    try {
      await reporter.multitenancyRepository.updateTenant(req.user.name, {
        $set: {
          windowsStoppedInformed: true
        }
      })
