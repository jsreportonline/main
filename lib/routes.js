...
module.exports = function (reporter) {
  ...
  app.post('/login', bodyParser.urlencoded({ extended: true, limit: '2mb' }), (req, res, next) => {
    ...
    passport.authenticate('local', (err, tenantOrUser, info) => {
      ...
      logger.debug(`login successfull ${tenantOrUser.isAdmin ? tenantOrUser.email : tenantOrUser.name} ${tenantOrUser.isAdmin} ${tenantOrUser.tenantId}`)
  ...
  app.use((req, res, next) => {
    ...
    if (tenant) {
      reporter.logger.info(`Processing ${req.method} ${req.path} ${tenant.email}`, { tenant: tenant.name })
    } else {
