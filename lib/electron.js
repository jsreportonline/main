        ...
        requestAsync({
          ...
          if (!httpResponse.body || httpResponse.statusCode !== 200) {
            if (httpResponse.body && httpResponse.body.toString().indexOf('socket hang up') !== -1) {
              throw new Error('The communication with your dedicated electron container crashed. This is usually caused by reaching provided resources limits or electron unexpected fail. The container is now about to be restarted.' + httpResponse.body)
            }

            throw new Error(httpResponse.body || 'elenctron rendering failed')
          }
