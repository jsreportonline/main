...
module.exports = (reporter, { reportsCleanupInterval = '1h' }) => {
  ...
  const tenantTreshold = (t) => {
    ...
    switch (t.plan || 'free') {
      ...
      case 'platinum': return 1000 * 60 * 60 * 24 * 7 // week
    ...
    try {
      ...
      for (const t of tenants) {
        try {
          ...
          const reportsToRemove = await reporter.documentStore.collection('reports').find({ creationDate: { $lt: removeOlderDate } }, { _id: 1 }).sort({ creationDate: 1 })
