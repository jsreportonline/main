...
module.exports = (reporter) => {
  ...
  return (opts, cb) => {
    ...
    return requestAsync({
      ...
      reporter.errorNotification('phantomjs error - ' + body.tenant, JSON.stringify(body), err)

      if (err.code === 'ECONNRESET' || err.message.indexOf('socket hang up') !== -1) {
        return cb(new Error('The communication with your dedicated phantomjs container crashed. This is usually caused by reaching provided resources limits or phantomjs unexpected fail. The container is now about to be restarted.' + err.message))
      }
