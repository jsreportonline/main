...
const isBillingDate = (tenant, now) => {
  ...
  if (tenant.nextBillingTimestamp != null) {
    nextBillingDate = moment(tenant.nextBillingTimestamp).toDate()
  } else {
    // support for old tenants with no next billing information registered
    nextBillingDate = getNextBillingDate(tenant.createdOn, tenant.lastBilledDate)
  }

  const nowWithNoTime = moment(now).startOf('day')

  if (nowWithNoTime.isSameOrAfter(nextBillingDate)) {
    return true
  }
...
const getNextBillingDate = module.exports.getNextBillingDate = (created, lastBilling) => {
  // date of month
  const billingDay = moment(created).date()

  let nDate = moment({
    day: 1,
    month: lastBilling ? lastBilling.getMonth() : created.getMonth(),
    year: lastBilling ? lastBilling.getFullYear() : created.getFullYear()
  }).add(1, 'months')

  const lastDayInNextMonth = moment(nDate).endOf('month').date()

  if (billingDay > lastDayInNextMonth) {
    nDate = nDate.date(lastDayInNextMonth)
  } else {
    nDate = nDate.date(billingDay)
  }
  ...
  if (shouldBillNow(tenant, now)) {
    return {
      $set: {
        creditsUsed: 0,
        lastBilledDate: now,
        // day with no time (timestamp in seconds)
        nextBillingTimestamp: moment(getNextBillingDate(tenant.createdOn, now)).startOf('day').valueOf()
      },
      $push: {
        billingHistory: {
          billedDate: now,
  ...
  setInterval(async () => {
    try {
      ...
      for (const t of tenants) {
        ...
        const now = new Date()
        const nowTimestampWithNoTime = moment(now).startOf('day').valueOf()
