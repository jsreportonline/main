...
module.exports = () => {
  ...
  jsreport.use(require('jsreport-debug')({
    // cant pass this through config file because of debug env already set
  ...
  return jsreport.init().then(function () {
    if (process.env.createIndexes) {
      return require('./indexes').ensureIndexes(jsreport.documentStore.provider.db, jsreport.multitenancyRepository.db)
    }
  }).then(function () {
    ...
    jsreport.documentStore.collection('templates').beforeInsertListeners.add('templatevalidation', (entity) => {
      if (!entity.name) {
        throw new Error('Template name must be filled')
      }
    })
